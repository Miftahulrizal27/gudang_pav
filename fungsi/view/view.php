...
class view
{
    ...
    {
        $sql = "SELECT barang.*,regional.nama_regional, kategori.nama_kategori,member.nm_member from penjualan 
                left join barang on barang.id_barang=penjualan.id_barang 
                left join regional on barang.id_regional = regional.id_regional
                left join kategori on barang.id_kategori = kategori.id_kategori
                left join member on member.id_member=penjualan.id_member
                WHERE penjualan.id_member = " . $_SESSION['admin']['id_member'] . "
    ...
    public function where_penjualan($id)
    {
        $sql = "SELECT barang.*,regional.nama_regional, kategori.nama_kategori,member.nm_member,penjualan.id_penjualan from penjualan 
                left join barang on barang.id_barang=penjualan.id_barang 
                left join regional on barang.id_regional = regional.id_regional
                left join kategori on barang.id_kategori = kategori.id_kategori
                left join member on member.id_member=penjualan.id_member
                WHERE penjualan.id_member = " . $_SESSION['admin']['id_member'] . "
                AND penjualan.id_barang = '" . $id . "'
                ORDER BY id_penjualan";
        $row = $this->db->prepare($sql);
        $row->execute();
        $hasil = $row->fetchAll();
